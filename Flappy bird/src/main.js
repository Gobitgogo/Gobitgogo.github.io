...
class Game {
    constructor() {
        ...
        let images = [
            "Img/spr_b3_strip4.png",
            "Img/spr_b2_strip4.png",
            "Img/spr_b1_strip4.png",
            'Img/all.png'
            ...
            "https://savepice.ru/uploaded/2018/6/16/56b7927f1b3ca11a50cd7942438e58d5-full.png"*/
        ];
        this.bgs = [];
        this.grs = [];
        this.birds = [];
        ...
        this.gbt.loadResourse(images)
            .then(([birdImg_1, birdImg_2, birdImg_3, allImg]) => {
                ...
                let pipeTopSprites = [
                    this.gbt.spriteSheet(allImg, 112, 646.5, 52, 319.5),
                    this.gbt.spriteSheet(allImg, 0, 646.5, 52, 319.5)
                ];
                let pipeBottomSprites = [
                    this.gbt.spriteSheet(allImg, 168, 646.5, 52, 319.5),
                    this.gbt.spriteSheet(allImg, 0, 646.5, 52, 319.5)
                ];
                let playbtnSprite = this.gbt.spriteSheet(allImg, 707, 236, 107, 58, this.changeForScreeningH(4), this.changeForScreeningH(8));
                let titleFlappyBirdSprite = this.gbt.spriteSheet(allImg, 700, 180, 180, 50, this.changeForScreeningH(2.5), this.changeForScreeningH(8));
                let titleGetReadySprite = this.gbt.spriteSheet(allImg, 590, 118, 185, 50, this.changeForScreeningH(2.5), this.changeForScreeningH(8));
                let titleGameOverSprite = this.gbt.spriteSheet(allImg, 790, 118, 195, 50, this.changeForScreeningH(2.5), this.changeForScreeningH(8));
                ...
                for (let i = 0; i < pipeTopSprites.length; i++) {
                    ...
                    for (let j = 0; j < 5; j++) {
                        ...
                        this.pipeTop[i][j] = new GameImage(
                            pipeTopSprites[i],
                            0,
                            0,
                        ...
                        this.pipeBottom[i][j] = new GameImage(
                            pipeBottomSprites[i],
                            0,
                            0,
                            this.changeForScreeningH(10),
                ...
                this.playbtn = new GameImage(
                    playbtnSprite,
                    0,
                    0,
                    playbtnSprite.width,
                    playbtnSprite.height)
                this.titleFlappyBird = new GameImage(
                    titleFlappyBirdSprite,
                    0,
                    0,
                    titleFlappyBirdSprite.width,
                    titleFlappyBirdSprite.height);
                this.titleGetReady = new GameImage(
                    titleGetReadySprite,
                ...
                this.titleGameOver = new GameImage(
                    titleGameOverSprite,
                    this.gameWidth / 2 - titleGetReadySprite.width / 2,
                    this.changeForScreeningH(8),
                    titleGetReadySprite.width,
                    titleGetReadySprite.height);
                this.tap = new GameImage(
                    tapSprite,
                    0,
                    0,
                    tapSprite.width,
...
class PlayStage {
    ...
    render(delta) {
        ...
        if (this.switchGame.sceneState === "gameOver") {
            game.paint.drawImage(game.titleGameOver);
        }
    ...
    update(delta) {
        ...
        if (game.gbt.onClick(game.gameScene)) {
            if (this.switchGame.sceneState === "start" ||
                ...
                this.switchGame.sceneState = "play";
                game.bird.vel.y = -game.changeForScreeningH(95);
                if (game.bird.angle > -90) {
                    game.bird.angle = -50;
                }
            }
            else if (this.switchGame.sceneState === "gameOver") {
                if (game.bird.y >= game.grs[0].y - game.bird.height) {
        ...
        if (this.switchGame.sceneState === "play") {
            for (let i = 0; i < this.pipeTop.length; i++) {
                ...
                if (game.gbt.collisionRect(game.bird, pT) || game.gbt.collisionRect(game.bird, pB)) {
                    ...
                    let rect = {x: 0, y: 0, width: game.gameWidth, height: game.gameHeight, fillColor: "white"}
                    game.paint.drawRect(rect);
    ...
    birdUpdate(delta) {
        game.bird.vel.y += delta * game.changeForScreeningH(30);
